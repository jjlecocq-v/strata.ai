...
| `committee-identity` | Put the authenticated member's real committee identity into the RLS-scoped app contract and shell | `baseline-rc`; current-member committee id | Payload/UI show `SP 6430 - 33 Malvern Avenue` and address; no generic building name; signed-out remains empty | S | `Display the authenticated committee identity` |
| `verify-committee-identity` | Independently attack identity/tenant rendering | Maker commit; two committee personas; signed-out persona | Contract/type test + Playwright proof of correct name for each tenant, no cross-tenant label, and signed-out lock | C run / fresh-eyes S | Same issue |
| `motion-audit-read` | Return the persisted audit rows already written for each motion | `baseline-rc`; audit schema | App-data selects/maps `motion_id`; drawer shows only that motion's events | C | `Load the persisted audit history for each motion` |
| `verify-motion-audit-read` | Prove create/open/approval/decide events survive fetch and isolation | Maker commit; local Supabase | Behavioural HTTP/UI journey plus cross-committee negative; no source-string-only pass | C run / fresh-eyes S | Same issue |
...
| `production-fallback` | Make locked verified fallback a valid Production mode without fixture substitution | Decision #5; current AI context/route | Production accepts explicit fallback; active-member RLS context only; bounded output visibly identified; live mode remains opt-in/fail-closed; README agrees | S | `Honor verified AI fallback mode in Production` |
| `verify-production-fallback` | Prove Production-mode config and adversarial data isolation | Maker commit; production-like build env; personas | Config/build test, signed-out/inactive denial, hidden-record denial, no mock app fixtures, no Gateway call, live-mode credential failure still non-2xx | S author / C run | Same issue |
| `live-capability-reconcile` | Apply the missing capability/attribution hardening exactly and non-destructively | Reviewed `202608160001…sql`; attested project; backup/recovery plan; isolated rehearsal | Migration ledger records one reviewed migration; expected helpers/search paths/triggers/grants/policies exist; old replaced policies absent | S ops | `Reconcile capability and attribution hardening on live Supabase` |
| `verify-live-capabilities` | Independently compare expected versus actual live security boundary | Repository migration; post-apply catalog; six persona matrix | Schema manifest diff passes; direct Data API/RLS negative matrix passes in isolated staging; Production gets non-mutating sentinels only | S | Same issue |
| `live-data-sanitize` | Remove/quarantine only known seed/test records from the real committee | Exact reviewed IDs; backup/export; operator targeted-cleanup unlock | JJ and committee retained; known test members and fixture cards no longer active/visible; no reset or committee-wide predicate | H+S ops | `Remove legacy fixture records from the SP 6430 workspace` |
| `verify-live-data` | Prove sanitation did not damage the real workspace | Before/after manifests | Committee/JJ/storage/law remain; motions are honestly empty; known titles/test emails absent; diff contains only approved IDs | Independent S | Same issue |
| `production-auth-config` | Prepare and apply the Production authentication boundary | Named Production hostname; Vercel target access; Supabase Auth access | Production Site URL/redirect allowlist covers sign-in, invite, `/recover`; leaked-password protection enabled if plan supports it; no wildcard surprise | H+S ops | `Configure Production auth redirects and password protection` |
| `verify-production-auth` | Inspect callbacks and recovery without mailbox dependence | Auth settings; Preview/Production origins | Read-back of exact allowlist; generated one-time recovery-link verifier on non-Production target; Production sentinel is non-mutating | Independent S | Same issue |
| `second-real-member` | Invite/add one genuine SP 6430 member | Real member email/name/role supplied by operator; stable approved origin | Invite sent once; person accepts; live member becomes active and linked; no seed identity counted | H | `Invite and activate a second real SP 6430 member` |
| `verify-second-real-member` | Confirm two independent humans and attributed permissions | JJ session; second-member session | Each sees only SP 6430; second member can record own approval; identities differ; invite callback succeeds | H, fresh eyes | Same issue |
| `candidate-preview` | Merge only passing source nodes and deploy an immutable Preview | Four passed source gates; clean branch | Preview URL, deployment ID, commit SHA, runtime attestation; no Production target/mutation | C ops | `Run the exact-candidate two-person Preview acceptance` |
| `secretary-preview-rehearsal` | Exercise the six-part flow against the exact candidate | Candidate Preview; passed live/auth/member gates | Signed-out lock; JJ sees exact committee and honest empty motions; attach/open; 0–0 failed probe; two-person passed/failed approval; tenant negative; clean console/a11y; evidence bundle | H + adversarial S | Same issue |
...
| `production-release` | Establish the dedicated committee Production surface | Explicit GO; exact candidate; configured auth; rollback target | Dedicated Production URL serves the named SHA; live fail-closed env; no Preview-share dependency; rollback procedure recorded | H+S ops | `Establish the dedicated Strata Production surface` |
| `production-smoke` | Independently verify the released boundary without destructive test data | Dedicated URL; two real sessions | Signed-out lock/recover; committee identity; existing accepted motion/document/approval evidence readable; nonmember denial; runtime attestation; clean console | H, fresh eyes | Same issue |
| `retire-tailadmin` | Remove or redirect the unrelated old public demo | Passed `production-smoke`; control of old Vercel project/domain | `strata-ai.vercel.app` redirects to the dedicated URL or no longer serves TailAdmin | H+S ops | `Retire the public TailAdmin Strata demo` |
| `verify-public-surface` | Check both public entry points from a signed-out browser | New and old URLs | New URL passes locked surface; old URL cannot expose TailAdmin; redirects have no loop; recovery remains reachable | C + fresh-eyes S | Same issue |
...
2. **Display the authenticated committee identity** (`committee-identity` + verifier).
3. **Load the persisted audit history for each motion** (`motion-audit-read` + verifier).
4. **Honor verified AI fallback mode in Production** (`production-fallback` + verifier).
5. **Reconcile capability and attribution hardening on live Supabase** (`live-capability-reconcile` + verifier).
6. **Remove legacy fixture records from the SP 6430 workspace** (`live-data-sanitize` + verifier).
7. **Configure Production auth redirects and password protection** (`production-auth-config` + verifier).
8. **Invite and activate a second real SP 6430 member** (`second-real-member` + verifier).
9. **Run the exact-candidate two-person Preview acceptance** (`candidate-preview` + `secretary-preview-rehearsal`).
10. **Establish the dedicated Strata Production surface** (`production-release` + `production-smoke`).
11. **Retire the public TailAdmin Strata demo** (`retire-tailadmin` + verifier).

Issue creation is delivery work for this audit. If GitHub remains unavailable, preserve the exact titles/contracts here, mark `issue-sync` blocked in state, and do not pretend the issues or PR exist.
